...
knitr::kable(table(d$engagement,d$region))
```
...
$$
  p_{i} = \mu + \rho_i + \epsilon_{i}
$$
where $p_i$ is the progressivism for person $i$, $\mu$ is the grand 
mean, $\rho_i$ is the factor that describes
...
knitr::kable(table(small.d$engagement,small.d$region))
```
...
To explore the relationship, we could build a data set where
...
have a more similar percentage of engagement 
(`r round(261/(261+293)*100)` vs `r round(72/(72+104)*100)`).
...
Washington Non Metro members. Thus the fraction of Thurston
members is `r round(sum(small.d$region=="Thurston")/nrow(small.d),3)`.
We have `r sum(small.d$engagement=="Highly Engaged")` Highly
Engaged members, so we'd expect that the number of Highly Engaged
members who also live in Thurston county would be 
`r round(sum(small.d$engagement=="Highly Engaged") * sum(small.d$region=="Thurston")/nrow(small.d),2)`. 
This is pretty close to the randomized value we calculated 
in the table above. 
...
In terms of the statistic, one easy one is the product of 
the diagonal elements minus the product of the off-diagonal 
elements. If this is close to zero, there isn't much association
between the factors. 

``` {r simulating_chi}

get.stat <- function(tbl) {
  val <- tbl[1,1] * tbl[2,2] - tbl[1,2] * tbl[2,1]
  return(val)
}

obs <- get.stat(table(small.d$region,small.d$engagement))

results <- data.frame(val=c(obs,rep(0,999))) 
...
print(mean(abs(results$val) >= abs(obs)))
```

This $p$-value, 0.184, is pretty similar to what we get by 
running `chisq.test`. 
